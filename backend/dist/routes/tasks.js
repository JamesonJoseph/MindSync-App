...
import { parseIsoDateTime, utcNow } from "../lib/time.js";
...
export const taskRoutes = async (app) => {
    ...
    app.post("/api/tasks", { preHandler: requireAuth }, async (request, reply) => {
        ...
        const eventDatetime = parseIsoDateTime(String(payload.event_datetime ?? "")) ?? utcNow();
        const reminderMinutes = Number(payload.reminder_minutes ?? 30);
        const reminderDatetime = parseIsoDateTime(String(payload.reminder_datetime ?? "")) ?? new Date(eventDatetime.getTime() - reminderMinutes * 60 * 1000);
    ...
    app.put("/api/tasks/:taskId", { preHandler: requireAuth }, async (request, reply) => {
        ...
        const eventDatetime = "event_datetime" in payload ? parseIsoDateTime(String(payload.event_datetime ?? "")) : null;
        ...
        const updated = await TaskModel.findOneAndUpdate({ _id: parseObjectId(String(request.params.taskId)), userId: request.auth.uid }, { $set: updates }, { new: true }).lean();
        if (!updated) {
    ...
    app.delete("/api/tasks/:taskId", { preHandler: requireAuth }, async (request, reply) => {
        const deleted = await TaskModel.deleteOne({
            _id: parseObjectId(String(request.params.taskId)),
            userId: request.auth.uid,
        });
