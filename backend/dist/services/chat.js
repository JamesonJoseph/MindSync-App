...
export async function runAssistantChat(input) {
    ...
    try {
        ...
        for (const toolCall of toolCalls) {
            ...
            let toolResponse = { error: "Unknown function" };
            ...
            if (functionName === "create_task") {
                const eventDatetime = args.event_datetime ? new Date(String(args.event_datetime)) : utcNow();
            ...
            if (functionName === "update_task") {
                const updated = await TaskModel.updateOne({ _id: String(args.task_id ?? ""), userId: input.uid }, { $set: { status: String(args.status ?? "pending") } });
