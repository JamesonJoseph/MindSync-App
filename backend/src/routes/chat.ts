...
export const chatRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/api/chat/save", { preHandler: requireAuth }, async (request, reply) => {
    ...
    let finalDoc;
    if (conversationId) {
      finalDoc = await ChatConversationModel.findOneAndUpdate(
        { _id: parseObjectId(conversationId), userId: request.auth.uid },
        {
          $set: conversationDoc,
    ...
    return serializeConversationDetail(finalDoc as Record<string, unknown>);
  });

  app.get("/api/chat/conversations", { preHandler: requireAuth }, async (request) => {
    const { limit = "100" } = request.query as Record<string, string | undefined>;
    const docs = await ChatConversationModel.find({ userId: request.auth.uid })
      .sort({ updatedAt: -1 })
      .limit(Math.min(Number(limit) || 100, 100))
      .lean();
    return docs.map((doc) => serializeConversationSummary(doc as Record<string, unknown>));
  });
  ...
  app.get("/api/chat/conversations/:conversationId", { preHandler: requireAuth }, async (request, reply) => {
    const doc = await ChatConversationModel.findOne({
      _id: parseObjectId(String((request.params as { conversationId: string }).conversationId)),
      userId: request.auth.uid,
    }).lean();
    ...
    return serializeConversationDetail(doc as Record<string, unknown>);
  });
  ...
  app.put("/api/chat/conversations/:conversationId", { preHandler: requireAuth }, async (request, reply) => {
    ...
    const updated = await ChatConversationModel.findOneAndUpdate(
      { _id: parseObjectId(String((request.params as { conversationId: string }).conversationId)), userId: request.auth.uid },
    ...
    return serializeConversationDetail(updated as Record<string, unknown>);
  });
