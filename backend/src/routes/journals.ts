...
export const journalRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/api/journals/search", { preHandler: requireAuth }, async (request, reply) => {
    const { startDate, endDate, q, limit = "500", sort = "desc" } = request.query as Record<string, string | undefined>;
    const mongoQuery: Record<string, unknown> = { userId: request.auth.uid };
  ...
  app.put("/api/journals/:journalId", { preHandler: requireAuth }, async (request, reply) => {
    ...
    const result = await JournalModel.findOneAndUpdate(
      { _id: parseObjectId(String((request.params as { journalId: string }).journalId)), userId: request.auth.uid },
      { $set: updates },
      { new: true },
    ).lean();
  ...
  app.delete("/api/journals/:journalId", { preHandler: requireAuth }, async (request, reply) => {
    const deleted = await JournalModel.deleteOne({
      _id: parseObjectId(String((request.params as { journalId: string }).journalId)),
      userId: request.auth.uid,
    });
